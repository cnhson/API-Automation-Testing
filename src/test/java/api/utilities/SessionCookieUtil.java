...
public class SessionCookieUtil {
    ...
    public static void storeToFile(String cookieString) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(sessionCookieFile.getAbsolutePath()))) {
            writer.println(cookieSession);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static String getFromFile() {
        try {
            String data = FileUtils.readFileToString(sessionCookieFile, "UTF-8");
            return data;
        } catch (IOException e) {
            e.printStackTrace();
            return null;
