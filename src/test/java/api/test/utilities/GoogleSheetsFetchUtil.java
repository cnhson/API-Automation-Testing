...
public class GoogleSheetsFetchUtil implements DataFetchInterface {
	...
	public GoogleSheetsFetchUtil(String sheetName) {
		try {
			...
			System.err.println("\n[GGSheetsFetchUtil] Error while init: " + e.getMessage());
		}
	}
	...
	public void setConfig() {
		try {
			...
			System.err.println("\n[GGSheetsFetchUtil] Error while setting config: " + e.getMessage());
		}
	}
		...
		try {
			...
			if (this.prevIndex < currentIndex) {
				...
				if (offset == null || ((currentIndex - 1) % this.chunk == 0)) {
					String sheetRange = this.sheetName + "!A" + (contentIndex + fetchTimes * this.chunk) + ":"
					...
					// Check if response size < chunk, add null value(s)
					Integer responseSize = this.response.getValues().size();
					if (this.maxRowIndex - currentIndex > this.chunk && responseSize < this.chunk) {
						for (Integer i = responseSize; i < this.chunk; i++) {
							this.response.getValues().add(i, new ArrayList<>());
						}
					}
					// System.out.println(this.response.getValues());
				}
			}
		}
		catch (Exception e) {
			System.err.println("\n[GGSheetsFetchUtil] Error while fetching data: " + e.getMessage());
		}
	}
	...
	public String[] getRowCellsData(Integer rowIndex, String... colNames) {
		try {
			...
			System.err.println("\n[GGSheetsFetchUtil] Error while getting cell value in sheet (" + sheetName
					+ "): " + e.getMessage());
			return null;
		}
	}
	...
	private ValueRange sheetDataRequest(String range) {
		...
		try {
			response = gs.buildSheetService().spreadsheets().values().get(this.spreadSheetID, range)
					.execute();
		}
		catch (IOException e) {
			e.printStackTrace();
			System.err.println("\n[GGSheetsFetchUtil] Error while getting sheet data, " + e.getMessage());
		}
		return response;
	}
	...
	public Boolean isCellNull(Integer rowIndex, Integer colIndex) {
		try {
			this.response.getValues().get(rowIndex).get(colIndex).toString();
			return false;
		}
		catch (Exception e) {
			return true;
	...
	public Boolean isFetchingEnd(Integer rowIndex) {
		try {
			...
			System.err.println("\n[GGSheetsFetchUtil] Error checking if fetching end: " + e.getMessage());
			System.out.println("\n[GGSheetsFetchUtil] Error in index: " + rowIndex);
			return false;
		}
	}
	...
	public void fetchClose() {
		try {
			...
			System.err.println("\n[GGSheetsFetchUtil] Error while closing: " + e.getMessage());
		}
	}
}
